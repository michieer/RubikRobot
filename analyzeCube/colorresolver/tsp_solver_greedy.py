"""
Traveling Salesman Problem solver using greedy algorithm
...
def pairs_by_dist(N: int, distances) -> Tuple[Tuple[int, int]]:
    ...
    sorted_indices = sorted(indices, key=lambda ix: distances[ix // N][ix % N])

    return tuple((ix // N, ix % N) for ix in sorted_indices)
