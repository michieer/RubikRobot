...
class AdjustServoFrame:
    ...
    def on_slider_change(self, value):
        #Update servo position in real-time as slider moves#
        value = int(float(value))
        self.value_label.config(text=f"Value: {value}")
        
        # Move servo in background thread to avoid blocking UI
        thread = threading.Thread(target=self._move_servo_safe, args=(value,), daemon=True)
        thread.start()
...
class CalibrationWindow:
    ...
    def __init__(self, parent_root):
        ...
        right_frame = tk.Frame(main_container, bg='#f0f0f0', relief="sunken", borderwidth=2, width=310, height=700)
        ...
        for direction in ['up', 'right', 'left', 'down']:
            frame = ttk.Frame(self.content_frame, relief="sunken", borderwidth=1)
        ...
        self.delays_content = ttk.Frame(self.content_frame, relief="sunken", borderwidth=1)
        self._create_delays_content_frame()
        
        # Bottom section: Adjustment window - fixed height and width
        self.adjust_container = ttk.Frame(right_frame, height=300, width=280)
